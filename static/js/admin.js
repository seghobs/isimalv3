function showAlert(message, type = 'success') {
    ...
    alert.innerHTML = `<i class="fas fa-${icon}"></i><span>${escapeHtml(String(message || ''))}</span>`;
...
function createTokenCard(token, index) {
    ...
    const safeAndroidId = escapeHtml(token.android_id_yeni || '');
    const safeDeviceId = escapeHtml(token.device_id || '');
    const safeLogoutReason = escapeHtml(token.logout_reason || '');
    const addedAt = token.added_at ? new Date(token.added_at).toLocaleString('tr-TR') : 'Bilinmiyor';
    const safeAddedAt = escapeHtml(addedAt);

    const fullNameDisplay = safeFullName ? `<div style="color: rgba(255, 255, 255, 0.6); font-size: 14px; margin-top: 5px;">${safeFullName}</div>` : '';
    ...
    const logoutReasonDisplay = token.logout_reason ? `
                <div style="background: rgba(231, 76, 60, 0.15); border: 1px solid rgba(231, 76, 60, 0.3); border-radius: 8px; padding: 12px; margin: 10px 0;">
                    ...
                    ${token.logout_time ? `<div style="color: rgba(255, 255, 255, 0.5); font-size: 11px; margin-top: 5px;">
                        ${escapeHtml(new Date(token.logout_time).toLocaleString('tr-TR'))}
                    </div>` : ''}
                </div>
            ` : '';
    ...
    card.innerHTML = `
                <div class="token-header">
                    <div>
                        <span class="token-username"><i class="fab fa-instagram"></i> @${safeUsername}</span>
                        ${fullNameDisplay}
                    </div>
                ...
                <div class="token-info">
                    <strong>Token:</strong>
                    <div class="token-value">${safeToken.substring(0, 60)}...</div>
                </div>
                
                <div class="token-info">
                    <strong>Android ID:</strong> <span class="token-value" style="display: inline-block;">${safeAndroidId}</span>
                </div>
                
                <div class="token-info">
                    <strong>Device ID:</strong> <span class="token-value" style="display: inline-block;">${safeDeviceId || '<em style="color:rgba(255,255,255,0.4)">Yok</em>'}</span>
                </div>
                
                <div class="token-info">
                    <strong>Eklenme Tarihi:</strong> ${safeAddedAt}
                </div>
...
document.getElementById('addTokenForm').addEventListener('submit', async (e) => {
    ...
    if (!formData.token || !formData.android_id || !formData.device_id || !formData.user_agent) {
...
if (changePasswordForm) {
    changePasswordForm.addEventListener('submit', async (e) => {
        ...
        const new_password_confirm = document.getElementById('new_password_confirm').value.trim();
        ...
        try {
            ...
            if (data.success) {
                ...
                showAlert(data.message || 'Şifre güncellenemedi', 'error');
            }
        } catch (error) {
            showAlert('Bir hata oluştu: ' + error.message, 'error');
        }
